...
elsif($task eq "post")
{
  ...
  my $gb2=$query->param("gb2");
	...
	post_stuff($parent,$name,$email,$gb2,$subject,$comment,$file,$file,$password,$nofile,$captcha,$admin,$no_captcha,$no_format,$postfix,$sage);
}
...
sub build_cache()
{
	...
	else
	{
		...
		my $realtotal=get_page_count_real(scalar @threads);
		my @pagethreads;
		while(@pagethreads=splice @threads,0,IMAGES_PER_PAGE)
		{
			if($page >= $total) {
				build_cache_page($page,$realtotal,@pagethreads);
			} else {
...
sub post_stuff($$$$$$$$$$$$$$)
{
  ...
  my $displayname = $uploadname;
  $displayname =~ s/^(.{20})(.+)(\..{1,7})$/$1\[...\]$3/;
  
  my $displaysize = $size;
  $displaysize = sprintf("%d Bytes", $size) if ($size<1024);
  $displaysize = sprintf("%.2f KB", $displaysize/1024) if ($displaysize>1024); #KB
  $displaysize = sprintf("%.2f MB", $displaysize/1024) if ($displaysize>1024); #MB
    ...
    if (sage_count($parent_res)>MAX_RES)
    {
      $sth=$dbh->prepare("UPDATE ".SQL_TABLE." SET autosage=1 WHERE num=? OR parent=?;"); # or make_error(S_SQLFAIL);
      $sth->execute($parent,$parent) or make_error(S_SQLFAIL);
    }
    if($admin)
    {
        $sth=$dbh->prepare("UPDATE ".SQL_TABLE." SET adminpost=1 WHERE parent=? ORDER BY num DESC LIMIT 1;"); # oh, you ugly fix :3
...
sub ban_check($$$$)
{
	...
	$sth2=$dbh->prepare("SELECT num,comment,ival2 FROM ".SQL_ADMIN_TABLE." WHERE type='ipban' AND ? & ival2 = ival1 & ival2;");
	$sth2->execute($numip) or make_error(S_SQLFAIL);
	while($row = $sth2->fetchrow_arrayref())
	{
		$id = $$row[0];
		$mask = dec_to_dot($$row[2]);
		$reason = $$row[1];
	}
	$_ip = new Net::IP ($ip.'/'.$mask);
	make_ban($ip, $_ip->prefix(), $reason, $_ip->size(), 0) if(($sth->fetchrow_array())[0]);
...
#	}
	...
	elsif($width>MAX_W or $height>MAX_H or THUMBNAIL_SMALL or $filename=~/\.svg$/)
	{
...
sub delete_stuff($$$$@)
{
	...
	make_error(S_BADDELPASS) unless((!$password and $deletebyip) or ($password and !$deletebyip) or $admin); # allow deletion by ip with empty password
	...
	foreach $post (@posts)
	{
		delete_post($post,$password,$fileonly,$archive,$deletebyip);
	}
...
sub delete_post($$$$$)
{
	...
	if($row=$sth->fetchrow_hashref())
	{
		...
		make_error(S_BADDELIP) if($deletebyip and ($ip and $$row{ip} ne $ip));
...
sub make_ban($$$$$)
{
	...
	make_http_header();
	if($mode == 1) {
		print encode_string(DNSBL_TEMPLATE->(ip=>$ip, dnsbl=>$reason));
	} else {
		print encode_string(BAN_TEMPLATE->(ip=>$ip, reason=>$reason, subnet=>$subnet, size=>$size));
	}
	if($dbh)
	{
		$dbh->{Warn}=0;
