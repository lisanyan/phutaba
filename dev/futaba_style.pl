...
<head>
<title><const TITLE> &raquo; <if $title><var $title></if><if !$title>/dev/ - Phutaba 1</if></title>
...
<link rel="stylesheet" title="Ernstchan" type="text/css" href="/css/ernstchan.css" />
<link rel="alternate stylesheet" title="Phutaba" type="text/css" href="/css/phutaba.css" />
<link rel="alternate stylesheet" title="Krautchan" type="text/css" href="/css/krautchan.css" />
<link rel="alternate stylesheet" title="Futaba" type="text/css" href="/css/futaba.css" />
...
use constant NORMAL_FOOT_INCLUDE => include("../globals/footer.html").q{
...
<if $postform>
	...
	<input type="submit" value="<const S_SUBMIT>" <if $locked>disabled="disabled"</if>/></td></tr>
		<tr><td class="postblock">Kontra</td><td><input type="checkbox" name="field2" value="sage" onclick="previewSage(this, 'sage', 'sageinfo')" />&nbsp;<span id="sage" class="sage"></span>&nbsp;<span id="sageinfo" class="sageinfo"></span></td></tr>
	<tr><td class="postblock"><const S_COMMENT></td><td><textarea id="field4" name="field4" cols="48" rows="6"></textarea> <img onclick="resizeCommentfield('field4', this)" src="/img/expand.png" title="Textfeld vergr&ouml;&szlig;ern"></td></tr>
  ...
  <hr />
	<loop $posts>
		<if !$parent>
		  ...
		  <span class="postername"><var $name></span><if $adminpost> <span class="adminpost">## Admin ##</span></if><if $trip><span class="postertrip"><var $trip></span></if>
			<var $date></label>
			<span class="reflink">
			...
			<if !$thread>[<a href="<var get_reply_link($num,0)>"><const S_REPLY></a>]</if>
				...
				<span class="filename">&nbsp;<const S_PICNAME><a target="_blank" title="<var $uploadname>" href="<var expand_image_filename($image)>/<var $uploadname>"><if !$uploadshort><var get_filename($image)></if><var $uploadshort></a>&nbsp;</span><br />
				...
				<if !$thumbnail>
					...
					<if !DELETED_THUMBNAIL>
						<div class="nothumb"><a target="_blank" href="<var expand_image_filename($image)>/<var $uploadname>"><const S_NOTHUMB></a></div>
					</if>
				</if>
				</div>
			</if>
		...
		<if $parent>
			...
			<span class="commentpostername"><var $name></span> <if $adminpost><span class="adminpost">## Admin ##</span></if><if $trip><span class="postertrip"><var $trip></span></if>
			<var $date></label>
			<span class="reflink">
			<if !$thread><a href="<var get_reply_link($parent,0)>#<var $num>">#</a>&nbsp;<a href="<var get_reply_link($parent,0)>#i<var $num>">Nr. <var $num></a>&nbsp;</if>
			<if $thread><a href="<var get_reply_link($parent,0)>#<var $num>">#</a>&nbsp;<a href="javascript:insert('&gt;&gt;<var $num>\n')">Nr. <var $num></a>&nbsp;</if>
			</span>
