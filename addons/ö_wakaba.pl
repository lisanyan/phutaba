...
BEGIN {
    require "../lib/kaiserchan_style.pl";
}
BEGIN {
    require "captcha.pl";
}
...
my $loc = $gi->country_code_by_addr($ip) or "unk";
...
sub post_stuff {
    ...
    check_captcha( $dbh, $captcha, $ip, $parent )
      if ( use_captcha(ENABLE_CAPTCHA, $loc) or (ENABLE_CAPTCHA and !$no_captcha and !is_trusted($trip)) );
...
#    $original_comment =~ s/\n/ /gm;
	...
	if(!$admin)
	{
	    ...
	    make_http_forward( encode('utf-8', HTML_SELF), ALTERNATE_REDIRECT ) if ( $parent eq '0' );
	    make_http_forward( encode('utf-8', HTML_SELF) . "?task=show&amp;thread=" . $parent,
	        ALTERNATE_REDIRECT )
	      if ( $c_gb2 =~ /thread/i );
	    make_http_forward( encode('utf-8', HTML_SELF), ALTERNATE_REDIRECT );
	}
	else
	{
		# forward back to moderation page
	    make_http_forward( encode('utf-8', HTML_SELF) ."?task=show&amp;page=0&amp;admin=$admin", ALTERNATE_REDIRECT ) if ( $parent eq '0' );
	    make_http_forward( encode('utf-8', HTML_SELF) . "?task=show&amp;thread=" . $parent . "&amp;admin=$admin",
	        ALTERNATE_REDIRECT )
	      if ( $c_gb2 =~ /thread/i );
	    make_http_forward( encode('utf-8', HTML_SELF) . "?task=show&amp;page=0&amp;admin=$admin", ALTERNATE_REDIRECT );
	}
}
...
sub delete_stuff {
    ...
    else { make_http_forward( encode('utf-8', HTML_SELF), ALTERNATE_REDIRECT ); }
}
