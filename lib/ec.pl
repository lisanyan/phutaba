#!/usr/bin/perl -X
# TODO
# * Rewrite the autoinstallation Code
# * Move the Error Handler View to futaba_style.pl
# * Rewrite output_json_stats
# * clean the whole file up
# * use only one wakaba.pl for whole ernstchan
...
my $rboard = ($query->param("board"));
$rboard = ( $query->url_param("board") ) unless $rboard;
...
if ( $json eq "post" ) {
	...
	if ( $board ne undef ) {
			output_json_stats($board);
	}
}

if (!$rboard) {
	make_error("Board nicht angegeben.");
}
if ($rboard ne undef) {
	$rboard = encode_entities(decode("utf8",$rboard));
	if(!table_exists("config_$rboard")) {
		make_error("Board /$rboard/ existiert nicht.");
	}
#	$sth = $
}
...
sub output_json_post {
    my ($id) = @_;
    $sth =
      $dbh->prepare( "SELECT date,parent,location,subject,displaysize,name,uploadname,width,num,size,tn_width,thumbnail,trip,adminpost,height,image,tn_height,comment,uploadshort FROM " . SQL_TABLE . " WHERE num=?;" );
    $sth->execute($id);
    my $utf8_encoded_json_text;
    my $row;
    if ( $row = $sth->fetchrow_hashref() ) {
        $$row{error_code} = 200;
        $utf8_encoded_json_text = encode_json($row);
    }
    else {
        $utf8_encoded_json_text = encode_json( { "error_code" => 400 } );
    }
    make_http_header();
...
# sub show_post
# shows an single post out of a thread, essential for the reloadless view of threads
...
sub show_page {
	...
	if(defined($admin))
	{
		check_password($admin, $config{'ADMIN_PASS'});
		$isAdmin = 1;
	}
    ...
    if ( !$row )    # no posts on the board!
    {
        ...
        my @pagethreads;
        my $built_page = 0;
        while ( @pagethreads = splice @threads, 0, $config{'IMAGES_PER_PAGE'}
            and $built_page == 0 )
        {
            if ( $page == $pageToShow ) {
...
sub get_page_count_real {
    my ($total) = @_;
    return int( ( $total + $config{'IMAGES_PER_PAGE'}- 1 ) / $config{'IMAGES_PER_PAGE'} );
}
...
sub get_file_meta {
    ...
    make_error("Specifiy a file please!") unless $file;
    make_http_header();
    $info = ImageInfo($file);
...
sub output_page {
    ...
    foreach my $thread (@threads) {
        ...
        if ( $$parent{locked} ) {
            $max_replies = $config{'REPLIES_PER_LOCKED_THREAD'};
            $max_images = ( $config{'IMAGE_REPLIES_PER_LOCKED_THREAD'} or $images );
        }

        # in case of a sticky thread, use custom number of replies
        # NOTE: has priority over locked thread
        if ( !$$parent{sticky_isnull} ) {
            $max_replies = $config{'REPLIES_PER_STICKY_THREAD'};
            $max_images = ( $config{'IMAGE_REPLIES_PER_STICKY_THREAD'} or $images );
        }
        ...
        foreach my $post ( @{ $$thread{posts} } ) {
            my $abbreviation =
              abbreviate_html( $$post{comment}, $config{'MAX_LINES_SHOWN'},
                $config{'APPROX_LINE_LENGTH'} );
            if ($abbreviation) {
    ...
    print(
        encode_string(
            PAGE_TEMPLATE->(
                postform => ( $config{'ALLOW_TEXTONLY'} or $config{'ALLOW_IMAGES'} ),
                image_inp    => $config{'ALLOW_IMAGES'},
                textonly_inp => ( $config{'ALLOW_IMAGES'} and $config{'ALLOW_TEXTONLY'} ),
                prevpage     => $prevpage,
                nextpage     => $nextpage,
                pages        => \@pages,
                threads      => \@threads,
    ...
    print(
        encode_string(
            PAGE_TEMPLATE->(
                ...
                postform     => ( $config{'ALLOW_TEXT_REPLIES'} or $config{'ALLOW_IMAGE_REPLIES'} ),
                image_inp    => $config{'ALLOW_IMAGE_REPLIES'},
                textonly_inp => 0,
...
sub add_secondary_images_to_row {
	...
	if ( $$row{uploadshort} )
	{
		$$row{uploadshort} = clean_string($$row{uploadshort});
	}


    if ( $$row{imageid_1} != 0 ) {
        my $sth2 = $dbh->prepare(
            "SELECT * FROM " . $config{'SQL_TABLE_IMG'} . " WHERE timestamp=?" )
          or make_error(S_SQLFAIL);
        ...
        $$row{uploadshort1} = clean_string($$res2{uploadshort});
    ...
    if ( $$row{imageid_2} != 0 ) {
        ...
        $$row{uploadshort2} = clean_string($$res2{uploadshort});
    ...
    if ( $$row{imageid_3} != 0 ) {
        ...
        $$row{uploadshort3} = clean_string($$res2{uploadshort});
...
sub print_page {
    ...
    if ($config{'USE_TEMPFILES'}) {
        my $tmpname = $config{'RES_DIR'} . 'tmp' . int( rand(1000000000) );
...
sub dnsbl_check {
    ...
    foreach my $dnsbl_info ( @{&DNSBL_INFOS} ) {
        ...
        my @ready = $sel->can_read($config{'DNSBL_TIMEOUT'});
        if (@ready) {
...
sub post_stuff {
    ...
    if ($admin)  # check admin password - allow both encrypted and non-encrypted
    {
        ...
        if ($parent) {
            ...
            make_error(S_NOTALLOWED) if ( $file  and !$config{'ALLOW_IMAGE_REPLIES'} );
            make_error(S_NOTALLOWED) if ( !$file and !$config{'ALLOW_TEXT_REPLIES'} );
        }
        else {
            make_error(S_NOTALLOWED) if ( $file  and !$config{'ALLOW_IMAGES'} );
            make_error(S_NOTALLOWED) if ( !$file and !$config{'ALLOW_TEXTONLY'} );
        }
    }
    ...
    make_error(S_TOOLONG) if ( length($name) > $config{'MAX_FIELD_LENGTH'} );
    make_error(S_TOOLONG) if ( length($email) > $config{'MAX_FIELD_LENGTH'} );
    make_error(S_TOOLONG) if ( length($subject) > $config{'MAX_FIELD_LENGTH'} );
    make_error(S_TOOLONG) if ( length($comment) > $config{'MAX_COMMENT_LENGTH'} );
    ...
    my $ssl = $ENV{HTTP_X_SSL};
    undef($ssl) unless $ssl;
    my $loc = $ENV{HTTP_X_GEO};
    undef($loc) unless $loc;
    ...
    dnsbl_check($ip) if ( !$whitelisted and $config{'ENABLE_DNSBL_CHECK'} );

    # process the tripcode - maybe the string should be decoded later
    my $trip;
    ( $name, $trip ) = process_tripcode( $name, $config{'TRIPKEY'}, $config{'SECRET'}, $config{'CHARSET'} );
    ...
    spam_engine(
        query           => $query,
        trap_fields     => $config{'SPAM_TRAP'} ? [ "name", "link" ] : [],
    ...
    check_captcha( $dbh, $captcha, $ip, $parent )
      if ( $config{'ENABLE_CAPTCHA'} and !$no_captcha and !is_trusted($trip) );

    # proxy check
    proxy_check($ip) if ( !$whitelisted and $config{'ENABLE_PROXY_CHECK'} );
    ...
    # format comment
    $comment =
      format_comment( clean_string( decode_string( $comment, $config{'CHARSET'} ) ) )
      unless $no_format;
    $comment .= $postfix;
...
#    $original_comment =~ s/\n/ /gm;
    ...
    # generate date
    my $date = make_date( $time, $config{'DATE_STYLE'} );

    # generate ID code if enabled
    $date .= ' ID:' . make_id_code( $ip, $time, $email ) if ($config{'DISPLAY_ID'});
    ...
    my $displayname = get_displayname($file);
    ...
    if ($file2) {
        ...
        my $displayname1 = get_displayname($file2);
        ...
        $sth2->execute(
            ...
            $width1,     $height1,   $thumbnail1,   $tn_width1,
            $tn_height1, $file2,     $displayname1, $displaysize1
        ) or make_error(S_SQLFAIL);
    }
    ...
    if ($file3) {
        ...
        my $displayname1 = get_displayname($file3);
        ...
        $sth2->execute(
            ...
            $width1,     $height1,   $thumbnail1,   $tn_width1,
            $tn_height1, $file3,     $displayname1, $displaysize1
        ) or make_error(S_SQLFAIL);
    }

    # finally, write to the database
    my $sth = $dbh->prepare(
        "INSERT INTO " . $config{'SQL_TABLE_IMG'} . "
    ...
    $sth->execute(
        ...
        $tn_height,   $isAdminPost,  $uploadname,          $displayname,
    ...
    if ($config{'ENABLE_IRC_NOTIFY'}) {
        my $sth = $dbh->prepare(
            "SELECT num FROM " . $config{'SQL_TABLE'} . " WHERE timestamp=? LIMIT 1;" )
          or make_error(S_SQLFAIL);
        $sth->execute($time) or make_error(S_SQLFAIL);
        my $row;
        if ( $row = $sth->fetchrow_hashref() ) {
            my $socket = new IO::Socket::INET(
                PeerAddr => $config{'IRC_NOTIFY_HOST'},
                PeerPort => $config{'IRC_NOTIFY_PORT'},
                Proto    => "tcp"
            );
            if ($socket) {
                if ( $parent and $config{'IRC_NOTIFY_ON_NEW_POST'} ) {
		      ...
		      . get_preview($original_comment) . "]\n";
                ...
                elsif ( !$parent and $config{'IRC_NOTIFY_ON_NEW_THREAD'} ) {
    ...
    if ($config{'ENABLE_WEBSOCKET_NOTIFY'}) {
        my $ufoporno = system('/usr/local/bin/push-post', $config{'BOARD_IDENT'}, $parent, $new_post_id, "2>&1", ">/dev/null");
    }
...
sub format_comment {
    ...
    if ( $config{'ENABLE_WAKABAMARK'} && $config{'ENABLE_BBCODE'} ) {
        make_error("only activate either BBCODE or WAKABAMARK");
    }

    if ($config{'ENABLE_WAKABAMARK'}) { $comment = do_wakabamark( $comment, $handler ) }
    elsif ($config{'ENABLE_BBCODE'}) { $comment = do_bbcode( $comment, $handler ) }
...
s{(https?://[^\s<>"]*?)((?:\s|<|>|"|\.|\)|\]|!|\?|,|&#44;|&quot;)*(?:[\s<>"]|$))}{\<a href="$1"\>$1\</a\>$2}sgi;
        ...
        $line =~ s!^(&gt;.*)$!\<span class="unkfunc"\>$1\</span\>!g
          unless ($config{'ENABLE_WAKABAMARK'});
...
sub make_anonymous {
    ...
    return S_ANONAME unless ($config{'SILLY_ANONYMOUS'});
...
#	}
    ...
    if ($config{'ENABLE_LOAD'}) {    # only called if files to be distributed across web
        $ENV{SCRIPT_NAME} =~ m!^(.*/)[^/]+$!;
        my $root = $1;
...
# prevent GHOST BUMPING by hanging a thread where it belongs: at the time of the last non sage post
            if ($config{'PREVENT_GHOST_BUMPING'}) {
...
sub make_admin_ban_panel {
    ...
    $sth =
      $dbh->prepare( "SELECT * FROM "
          . SQL_ADMIN_TABLE
          . " WHERE type='ipban' OR type='wordban' OR type='whitelist' OR type='trust' ORDER BY type ASC,num ASC;"
      ) or make_error(S_SQLFAIL);
    $sth->execute() or make_error(S_SQLFAIL);
    while ( $row = get_decoded_hashref($sth) ) {
...
"<br /><span class="user_banned">(USER WURDE F&Uuml;R DIESEN POST GESPERRT)</span>";
    ...
    $sth = $dbh->prepare(
        "INSERT INTO " . SQL_ADMIN_TABLE . " VALUES(null,?,?,?,?,?);" )
      or make_error(S_SQLFAIL);
    $sth->execute( $type, $comment, $ival1, $ival2, $sval1 )
      or make_error(S_SQLFAIL);
    if ($postid) {
        $sth =
...
sub expand_image_filename {
    ...
    return expand_filename( clean_path($filename) ) unless $config{'ENABLE_LOAD'};
...
sub init_database {
    ...
    $sth = $dbh->prepare(
          ...
          "displayname TEXT," .    # Thumbnail height in pixels
