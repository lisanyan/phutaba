...
<if $postform>
	...
	<if !($$cfg{FORCED_ANON}) or $isAdmin><tr><td class="postblock"><label for="name"><const S_NAME></label></td><td><input type="text" name="nya1" id="name" /></td></tr></if>

	<tr><td class="postblock"><label for="subject"><const S_SUBJECT></label></td><td><input type="text" name="nya3" id="subject" />
	<input type="submit" id="postform_submit" value="<if $thread><const S_BTREPLY> /<var $$cfg{SELFPATH}>/<var $thread></if><if !$thread><const S_BTNEWTHREAD></if>" /></td>
	</tr>

	<if $thread>
	<tr><td class="postblock"><label for="sage"><const S_SAGE></label></td>
	<td><label><input type="checkbox" name="nya2" value="sage" id="sage" /> <const S_SAGEDESC></label></td>
	</tr>
	</if>

	<tr><td class="postblock"><label for="field4"><const S_COMMENT></label></td>
	<td><textarea id="field4" name="nya4" cols="48" rows="6"></textarea> <img onclick="resizeCommentfield('field4', this)" src="/img/icons/expand.png" alt="<const S_IMGEXPAND>" title="<const S_IMGEXPAND>" />
	</td></tr>
	...
	<tr><td class="postblock"><label for="nopomf"><const S_NOPOMF></label></td>
	<td><label><input type="checkbox" name="no_pomf" value="on" id="nopomf" onclick="Settings.set('nopomf', +this.checked);" /> <const S_NOPOMFDESC></label></td>
	</tr>
	</if>
	...
	<if need_captcha($$cfg{CAPTCHA_MODE}, $$cfg{CAPTCHA_SKIP}, $loc)>
		<tr><td class="postblock"><const S_CAPTCHA> (<a href="/faq">?</a>) (<var $loc>)</td><td><input type="text" name="captcha" size="10" /> <img alt="" onclick="update_captcha(this);" id="imgcaptcha" src="/captcha.pl?key=<var get_captcha_key($thread)>&amp;dummy=<var $dummy>&amp;board=<var $$cfg{SELFPATH}>" /></td></tr>
	</if>
...
<if !$thread>
	<nav>
		...
		<ul class="pagelist">
			<li>[<a href="#top" id="bottom"><const S_TOP></a>]</li>
		</ul>
	</nav>
</if>
<if $thread>
